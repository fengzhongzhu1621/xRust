...
use core_utils::platform::windows::{
    clipboard, constants::*, dc::*, mem::*, system::*, types::*, utils::*,
};
...
pub fn get_string(out: &mut alloc::vec::Vec<u8>) -> SysResult<usize> {
    ...
    let ptr = RawMem::from_borrowed(clipboard::get_clipboard_data(
    ...
    let result = unsafe {
        ...
        if let Some(null_idx) =
            out.iter().skip(storage_cursor).position(|b| *b == b'\0')
        {
...
pub fn get_file_list_path(
    out: &mut alloc::vec::Vec<std::path::PathBuf>,
) -> SysResult<usize> {
    use std::os::windows::ffi::OsStringExt;

    let clipboard_data = RawMem::from_borrowed(clipboard::get_clipboard_data(
        clipboard::CF_HDROP,
    )?);
    ...
    let num_files = unsafe {
        DragQueryFileW(clipboard_data.get() as _, u32::MAX, ptr::null_mut(), 0)
    };
    ...
    for idx in 0..num_files {
        ...
        let required_size_no_null = unsafe {
            DragQueryFileW(clipboard_data.get() as _, idx, ptr::null_mut(), 0)
        };
...
pub fn get_bitmap(out: &mut alloc::vec::Vec<u8>) -> SysResult<usize> {
    ...
    let header_storage = RawMem::new_rust_mem(if clr_bits < 24 {
        mem::size_of::<BITMAPINFOHEADER>()
            + mem::size_of::<RGBQUAD>() * (1 << clr_bits)
    } else {
        mem::size_of::<BITMAPINFOHEADER>()
    })?;
    ...
    header.bmiHeader.biSizeImage =
        ((((header.bmiHeader.biWidth * clr_bits + 31) & !31) / 8)
            * header.bmiHeader.biHeight) as _;
    ...
    for color in unsafe {
        slice::from_raw_parts(
            header.bmiColors.as_ptr(),
            header.bmiHeader.biClrUsed as _,
        )
    } {
...
pub fn set_without_clear(format: u32, data: &[u8]) -> SysResult<()> {
...

    {
        ...
        // 拷贝内容到全局内存对象
        unsafe {
            ptr::copy_nonoverlapping(data.as_ptr(), ptr.as_ptr() as _, size)
        };
...
pub fn set_string(data: &str) -> SysResult<()> {
    ...
    if size != 0 || data.is_empty() {
        let mem = RawMem::new_global_mem(
            (mem::size_of::<u16>() * (size as usize + 1)) as _,
        )?;
        {
            let (ptr, _lock) = mem.lock()?;
        ...
        if unsafe {
            !SetClipboardData(clipboard::CF_UNICODETEXT, mem.get()).is_null()
        } {
            //SetClipboardData takes ownership
            mem.release();
            return Ok(());
