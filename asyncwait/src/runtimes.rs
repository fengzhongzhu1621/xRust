...
pub fn tokio_async() {
    ...
    rt.block_on(async {
        ...
        rt.spawn(async {
            println!("tokio hello from a tokio task!");
            println!("tokio in spawn")
        })
        .await
        .unwrap();
    });

    rt.spawn_blocking(|| println!("tokio in spawn_blocking"));
}

pub fn futures_async() {
    // 创建线程池
    let pool = ThreadPool::new().expect("executor failed to build pool");
    // 创建一个无上限容量的通道
    ...
    let fut_values = async {
        let fut_tx_result = async move {
            (0..100).for_each(|v| {
                tx.unbounded_send(v)
                    .expect("executor channel failed to send");
            })
        };
    ...
    println!("executor values={:?}", values);
}
    ...
    futures_lite::future::block_on(async {
        ...
        loop {
            select! {
                a = a_fut => total += a,
                b = b_fut => total += b,
                complete => {println!("futures_select complete"); break},
